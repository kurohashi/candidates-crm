...
function init() {
	...
	for (var i in conf.sys.versions) {
		app.use("/" + conf.sys.versions[i], auth.isAuthenticated, require("./routes/auth." + conf.sys.versions[i]), utils.authFailed);
		app.use("/open/" + conf.sys.versions[i], require("./routes/open." + conf.sys.versions[i]));
	}
